...
export async function getAnalytics(db) {
  const { data: rows, error } = await db
    .from("keys")
    .select("id, product_id, status, email, sold_at, stripe_session_id, key_code_encrypted")
  ...
  const stock = {};
  for (const id of PRODUCT_IDS) {
    stock[id] = { available: 0, sold: 0 };
  }
  const soldByProductMap = {};
  ...
  const days30Ms = now - 30 * 24 * 60 * 60 * 1000;
  const days7Ms = now - 7 * 24 * 60 * 60 * 1000;

  for (const row of rows || []) {
    if (row.status === "available") {
      stock[row.product_id] = stock[row.product_id] || { available: 0, sold: 0 };
      stock[row.product_id].available += 1;
      totalAvailable += 1;
      continue;
    }
    if (row.status === "sold") {
      stock[row.product_id] = stock[row.product_id] || { available: 0, sold: 0 };
      stock[row.product_id].sold += 1;
      totalSold += 1;
      soldByProductMap[row.product_id] = (soldByProductMap[row.product_id] || 0) + 1;

      const soldAt = row.sold_at ? new Date(row.sold_at).getTime() : NaN;
      if (Number.isFinite(soldAt)) {
        if (soldAt >= todayMs) salesToday += 1;
        if (soldAt >= days30Ms) salesLast30Days += 1;
        if (soldAt >= days7Ms) {
          const dayKey = new Date(soldAt).toISOString().slice(0, 10);
  ...
  const soldByProduct = Object.entries(soldByProductMap)
    .map(([product_id, count]) => ({ product_id, count }))
    .sort((a, b) => a.product_id.localeCompare(b.product_id));

  const salesLast7Days = [...salesByDay.entries()]
  ...
  return {
    stock,
    totals: { available: totalAvailable, sold: totalSold },
    soldByProduct,
    salesLast7Days,
    salesLast30Days,
    salesToday,
